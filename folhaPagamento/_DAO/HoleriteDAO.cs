...
{
    internal class HoleriteDAO : ConexaoDB
    {
        private List<Holerite> folhapagamento;

        public HoleriteDAO()
        {
            folhapagamento = new List<Holerite>();
        }

        public List<Holerite> CarregaHolerite()
        {
            folhapagamento.Clear();
            string sql = HoleriteSQL.CarregaHolerite;

            using (NpgsqlConnection conn = new NpgsqlConnection(stringConexao()))
            {
                conn.Open();
